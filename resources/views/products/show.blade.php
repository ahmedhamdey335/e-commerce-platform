        ...
        <div class="lg:grid lg:grid-cols-2 lg:gap-x-12 xl:gap-x-16">
            ...
            <div class="mt-10 px-4 sm:px-0 lg:mt-0">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                    <a href="{{ route('products.index', request()->only('category', 'search')) }}" class="inline-flex items-center text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                        Back to products
                    </a>
                    <span class="text-sm text-gray-500 dark:text-gray-400">Unique item from {{ $product->seller->name ?? 'our marketplace' }}</span>
                </div>
                ...
                <div class="mt-6 border-t border-gray-200 dark:border-gray-700 pt-6">
                    ...
                    @if($product->seller)
                        <div class="mt-4">
                            <a href="{{ route('products.index', ['seller' => $product->seller->id]) }}" class="text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 transition-colors">View more products from {{ $product->seller->name }}</a>
                        </div>
                    @endif
                </div>
